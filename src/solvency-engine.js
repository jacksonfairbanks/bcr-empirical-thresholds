 ...
 * Returns daily bcrPath and navPath so BCR visibly lags price (step-down on payment days).
 *
 * Cadence:
 ...
 * @param {number} annualObligation - annual dividend obligation in same units as nav (used to compute BCR)
 * @param {string} [cadence='monthly'] - 'monthly' or 'daily'
 *
 * Returns { terminalBCR, terminalNAV, minBCR, failed, earliestPrimaryFailureDay,
 *           bcrPath, navPath, btcHoldPath }.
...
function runSolvencyOnDailyPath(dailyPrices, dailyDates, btcStart, paymentObligation, annualObligation, cadence) {
    ...
    for (let i = 0; i < dailyPrices.length; i++) {
        ...
        if (!failed) {
            ...
            const nav = btcHold * price;
            const bcr = annualObligation > 0 ? nav / annualObligation : 0;
            navPath.push(nav);
            bcrPath.push(bcr);
            ...
            navPath.push(navPath[navPath.length - 1]);
    ...
    return {
        ...
        terminalNAV: navPath[navPath.length - 1],
