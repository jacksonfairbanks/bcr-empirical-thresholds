# bcr-empirical-thresholds

Replication artifact for the paper **"Empirical Coverage Thresholds for Perpetual Preferred Against Bitcoin: A stripped-model backtest using the Bitcoin Coverage Ratio across observed Bitcoin drawdowns"** (Jackson Fairbanks, 2026).

Repository: https://github.com/jacksonfairbanks/bcr-empirical-thresholds
License: MIT (see `LICENSE`)

## What this is

The paper measures the Bitcoin Coverage Ratio (BCR = BTC reserve value / annual dividend obligation) at which a stripped-model perpetual preferred issuer clears observed Bitcoin drawdowns. It applies the stripped model to four real BTC drawdown regimes (April 2013, Mt. Gox 2014, ICO Unwind 2018, Fed Tightening 2022) plus a 2011 supplementary regime, sweeps 245 starting-BCR values per regime, and records primary (BCR<1x) and terminal-liquidity thresholds at 0.1x resolution.
...
```
git clone https://github.com/jacksonfairbanks/bcr-empirical-thresholds.git
cd bcr-empirical-thresholds
...
There is no daily surface; do not run `build-surface.js --cadence=daily` (the flag has no effect there: the surface is constructed at monthly cadence only, per paper §6.5).

`bcr_3d_viewer.html` is a self-contained interactive view of the §6.5 coverage surface (Plotly via CDN). Open it in any modern browser; no server needed. The viewer's embedded data is a snapshot of `outputs/surface_grid.csv` and is regenerable byte-for-byte via `node scripts/bake-viewer.js`. After re-running `build-surface.js`, re-bake the viewer and refresh its hash in `verify.sh`.
...
├── bcr_3d_viewer.html                       interactive 3D coverage surface (Plotly, snapshot of outputs/surface_grid.csv; paper §6.5)
├── src/
...
│   ├── build-surface-grid-validation.js     canonical step 4: §6.5.2 body comparison (grid calendar start, 2017-12-16)
...
├── figures/                                 paper-cited canonical PNGs (fig01-05, fig07, fig09, figB1, figB2)
├── data/
...
└── outputs/
    ...
    ├── surface_grid_validation.csv          four-regime §6.5.2 body comparison (grid calendar start, 2017-12-16)
...
```
Fairbanks, J. (2026). Empirical Coverage Thresholds for Perpetual Preferred Against Bitcoin:
A stripped-model backtest using the Bitcoin Coverage Ratio across observed Bitcoin drawdowns.
```

## Contact

Replication issues and code-level bugs: <https://github.com/jacksonfairbanks/bcr-empirical-thresholds/issues>
