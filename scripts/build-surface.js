...
const SURFACE_CODE_VERSION = 'v1.0.0';
...
const NAV_INITIAL_USD = runnerMod.NAV_INITIAL_USD;
...
function clearsPrimary(startingBcr, prices, dates) {
    ...
    const btc0 = NAV_INITIAL_USD / peakPrice;
    const annualObligationUsd = NAV_INITIAL_USD / startingBcr;
...
function main() {
    ...
    const validationCols = ['regime', 'drawdown_fraction', 'duration_days',
                            'historical_primary_threshold', 'synthetic_primary_threshold',
    ...
    for (const rid of VALIDATION_REGIME_IDS) {
        ...
        const histThreshold = Number(desc.primary_threshold_hi);
        ...
        if (pq == null) {
            ...
            validationRows.push({
                ...
                historical_primary_threshold: histThreshold,
                synthetic_primary_threshold: null,
                delta_bcr: null,
                abs_delta_bcr: null,
                in_pool: POOL_REGIME_IDS.includes(rid),
            });
        ...
        validationRows.push({
            ...
            duration_days: duration,
            historical_primary_threshold: histThreshold,
            synthetic_primary_threshold: synThreshold,
            delta_bcr: delta,
    ...
    const surfaceCols = ['drawdown_fraction', 'duration_days',
                         'starting_bcr_primary_threshold',
                         'starting_bcr_terminal_threshold',
                         'synthesis_status'];
    const surfaceRows = [];
        ...
        for (const dur of durValues) {
            ...
            if (dur === 0) {
                row = {
                    ...
                    duration_days: dur,
                    starting_bcr_primary_threshold: null,
                    starting_bcr_terminal_threshold: null,
            ...
            if (res.status === 'ok') {
                row = {
                    drawdown_fraction: dd,
                    duration_days: dur,
                    starting_bcr_primary_threshold: res.threshold,
                    starting_bcr_terminal_threshold: null,
                    synthesis_status: 'ok',
                };
            } else {
