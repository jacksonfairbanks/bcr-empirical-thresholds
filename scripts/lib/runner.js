...
const NAV_INITIAL_USD = 1_000_000_000;
const DIV_RATE = 0.10;
...
function runScenario(regimeId, startingBcr, regimeStrip, regimeDates, regimePrices, cadence) {
    ...
    const { bcrPath, navPath, btcHoldPath, earliestPrimaryFailureDay } = res;

    let recoveryDay = -1;
    let sawDrawdown = false;
    for (let i = 0; i < n; i++) {
        if (!sawDrawdown) {
            if (navPath[i] < NAV_INITIAL_USD) sawDrawdown = true;
        } else {
            if (navPath[i] >= NAV_INITIAL_USD) { recoveryDay = i; break; }
        }
    }
    ...
    for (let i = 0; i <= endIdx; i++) {
        ...
        const navIndex = navPath[i] / NAV_INITIAL_USD;
    ...
    return {
        ...
        terminalNavIndex: navPath[endIdx] / NAV_INITIAL_USD,
...
const LONG_CSV_COLUMNS = [
    ...
    'nav_index', 'btc_held_index',
    'nav_usd', 'btc_held', 'btc_value_usd',
...
module.exports = { buildRegimeStrip, runScenario, LONG_CSV_COLUMNS, NAV_INITIAL_USD, DIV_RATE };
