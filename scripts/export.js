    ...
    return {
        ...
        failure_definitions: {
            ...
            terminal_state_semantics: {
                ...
                recovered:      "path's NAV returned to initial NAV (equivalently BCR returned to starting_bcr) without failing or going terminal",
        ...
        metrics: {
            ...
            primary_to_terminal_gap_days: {
                definition: "calendar days between primary failure (BCR < 1x) and terminal liquidity on the same path. Paper §4.3, §6.3.",
        ...
        estimators: {
            ...
            dividend_convention: {
                ...
                annualization: "annual_obligation_usd = nav_initial_usd / starting_bcr (equivalently nav_initial_usd * amplification * div_rate with amplification = 1 / (starting_bcr * div_rate))",
        ...
        dollar_anchor: {
            nav_initial_usd: runnerMod.NAV_INITIAL_USD,
            div_rate: runnerMod.DIV_RATE,
            note: "Dollar columns are constructed from a $1B initial NAV and 10% dividend rate. All dimensionless columns (nav_index, btc_held_index, bitcoin_coverage_ratio, *_fraction, *_over_annual) are invariant to this anchor."
        },
        ...
        pipeline_config: {
            ...
            truncation_rule: "slice at first recovery day (first day NAV returns to NAV_0 after an initial dip below) if recovered; otherwise slice at regime window end. Terminal liquidity fires inside the slice or not at all -- the slice is not extended to chase it.",
            ...
            recovery_semantics: "recoveryDay = first day nav_index >= 1.0 that occurs after at least one earlier day with nav_index < 1.0. Day 0 (nav_index = 1.0 trivially) never counts as recovery."
        },
        ...
        regime_results: ctx.regimeDescriptors.map(d => ({
            ...
            primary_threshold_lo: d.primary_threshold_lo,
            primary_threshold_hi: d.primary_threshold_hi,
            terminal_threshold_lo: d.terminal_threshold_lo,
            terminal_threshold_hi: d.terminal_threshold_hi
        })),
        ...
        data_dictionary: {
            ...
            renames_from_prior_drafts: {
                ...
                solvency_to_liquidity_gap_days: "primary_to_terminal_gap_days (paper §4.3 / §6.3 vocabulary)",
                failure_threshold_bcr1x_lo: "primary_threshold_lo",
                failure_threshold_bcr1x_hi: "primary_threshold_hi",
                failure_threshold_terminal_lo: "terminal_threshold_lo",
                failure_threshold_terminal_hi: "terminal_threshold_hi",
        ...
        out_of_scope: [
            ...
            "behavioral and par-trading thresholds (paper §4.4)",
...
function writeTransitionPathsCsv(ctx) {
    ...
    for (const desc of ctx.regimeDescriptors) {
        const lo = desc.primary_threshold_lo, hi = desc.primary_threshold_hi;
        const tlo = desc.terminal_threshold_lo, thi = desc.terminal_threshold_hi;
